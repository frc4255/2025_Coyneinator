...
public class RobotContainer {
    private static final boolean ENABLE_SUPERSTRUCTURE = false;
    ...
    private final Joystick operator = ENABLE_SUPERSTRUCTURE ? new Joystick(1) : null;
    ...
    private final JoystickButton manualWristRoll =
        ENABLE_SUPERSTRUCTURE && operator != null
            ? new JoystickButton(operator, XboxController.Button.kA.value)
            : null;
    private final JoystickButton zeroWristRoll =
        ENABLE_SUPERSTRUCTURE && operator != null
            ? new JoystickButton(operator, XboxController.Button.kBack.value)
            : null;

    private final JoystickButton coralHPIntake =
        ENABLE_SUPERSTRUCTURE && operator != null
        ...
        public RobotContainer() {
            ...
            if (ENABLE_SUPERSTRUCTURE) {
                ...
                s_Pivot = null;
                s_Elevator = null;
                s_DifferentialWrist = null;
                s_EndEffector = null;
                s_Climber = null;
                manager = null;
                pieceSensors = null;
                supervisor = null;
                superstructureVisualizer = null;
            }
        ...
        public void setManagerAsInactive () {
            if (!ENABLE_SUPERSTRUCTURE || supervisor == null || manager == null) {
                s_GroundIntake.stop();
        ...
        public void updateManager() {
            if (!ENABLE_SUPERSTRUCTURE || manager == null || supervisor == null) {
                return;
            }
            manager.update();
            supervisor.periodic();
        ...
        public void updateVisualizer() {
            if (!ENABLE_SUPERSTRUCTURE || superstructureVisualizer == null) {
                return;
            }
        ...
        private void configureButtonBindings() {
            ...
            if (!ENABLE_SUPERSTRUCTURE) {
                ...
                outtakeTrigger.whileTrue(
                    Commands.startEnd(
                        () -> s_GroundIntake.setHandoffSpeeds(),
                        () -> s_GroundIntake.stopRollers(),
                        s_GroundIntake
                    )
                );

                driver.a().onTrue(Commands.runOnce(s_GroundIntake::setAsHomed, s_GroundIntake));
                driver.y().onTrue(Commands.runOnce(() -> s_GroundIntake.setPitchGoal(0.0), s_GroundIntake));

                driver.rightBumper().onTrue(
                    Commands.runOnce(
                        () -> s_GroundIntake.setPitchGoal(
                            s_GroundIntake.getPitchGoalPosition() + Units.degreesToRadians(5.0)
                        ),
                        s_GroundIntake
                    )
                );
