...
public class TrajectoryLibrary {
    ...
    public Optional<PathPlannerPath> loadPath(String pathName) {
        if (pathCache.containsKey(pathName)) {
            return Optional.ofNullable(pathCache.get(pathName));
        }
        ...
        try {
            ...
            DriverStation.reportError("Failed to load PathPlanner path: " + pathName, e.getStackTrace());
    ...
    public Pose2d getAllianceAwareStartPose(PathPlannerPath path) {
        if (path == null) {
            return new Pose2d();
        }
        ...
        if (alliance.isPresent() && alliance.get() == Alliance.Red) {
            return path.flipPath().getStartingDifferentialPose();
        }
        ...
        return loadPath(pathName)
            .<Command>map(path ->
                new SequentialCommandGroup(
                    new InstantCommand(
                        () -> swerve.setPose(getAllianceAwareStartPose(path)),
                        swerve
                    ),
                    swerve.followPathCommand(path)
                )
            )
            .orElseGet(() -> Commands.print("Path not available: " + pathName));
    }
